# SPDX-FileCopyrightText: Copyright (c) 2025-2026, NVIDIA CORPORATION & AFFILIATES. All rights reserved.
# SPDX-License-Identifier: Apache-2.0

# This file was generated by stubgen-pyx v0.2.18 from python/pylibcudf/pylibcudf/io/parquet_metadata.pyx

from pylibcudf.io.types import SourceInfo
from pylibcudf.table import Table
from pylibcudf.types import DataType
from pylibcudf.typing import CudaStreamLike
from rmm.pylibrmm.memory_resource import DeviceMemoryResource
from typing_extensions import Buffer

const_unique_ptr_datasource = ...
const_string = ...
const_cpp_FileMetaData = ...
__all__ = ['ColumnChunk', 'ColumnChunkMetaData', 'ColumnChunkStatistics', 'FileMetaData', 'ParquetColumnSchema', 'ParquetMetadata', 'ParquetSchema', 'RowGroup', 'SortingColumn', 'read_parquet_column_chunk_bounds', 'read_parquet_footers', 'read_parquet_metadata']

class ParquetColumnSchema:
    def __init__(self): ...
    def name(self) -> str: ...
    def num_children(self) -> int: ...
    def child(self, idx: int) -> ParquetColumnSchema: ...
    def children(self) -> list[object]: ...
    def cudf_type(self) -> DataType: ...

class ParquetSchema:
    def __init__(self): ...
    def root(self) -> ParquetColumnSchema: ...
    def column_types(self) -> dict[object, object]: ...

class ParquetMetadata:
    def __init__(self): ...
    def schema(self) -> ParquetSchema: ...
    def num_rows(self) -> int: ...
    def num_rowgroups(self) -> int: ...
    def num_rowgroups_per_file(self) -> list[int]: ...
    def metadata(self) -> dict[object, object]: ...
    def rowgroup_metadata(self) -> list[object]: ...
    def columnchunk_metadata(self) -> dict[object, list[int]]: ...

class SortingColumn:
    def __init__(self): ...
    @property
    def column_idx(self) -> int: ...
    @property
    def descending(self) -> bool: ...
    @property
    def nulls_first(self) -> bool: ...

class ColumnChunkStatistics:
    def __init__(self): ...
    @property
    def has_min_max(self) -> bool: ...
    @property
    def min_encoded(self) -> bytes | None: ...
    @property
    def max_encoded(self) -> bytes | None: ...
    @property
    def null_count(self) -> int | None: ...
    @property
    def distinct_count(self) -> int | None: ...
    @property
    def is_min_value_exact(self) -> bool | None: ...
    @property
    def is_max_value_exact(self) -> bool | None: ...

class ColumnChunk:
    def __init__(self): ...
    @property
    def file_path(self) -> str: ...
    @property
    def file_offset(self) -> int: ...
    @property
    def offset_index_offset(self) -> int: ...
    @property
    def offset_index_length(self) -> int: ...
    @property
    def column_index_offset(self) -> int: ...
    @property
    def column_index_length(self) -> int: ...
    @property
    def schema_idx(self) -> int: ...
    @property
    def meta_data(self) -> ColumnChunkMetaData: ...

class ColumnChunkMetaData:
    def __init__(self): ...
    @property
    def path_in_schema(self) -> list[str]: ...
    @property
    def num_values(self) -> int: ...
    @property
    def total_uncompressed_size(self) -> int: ...
    @property
    def total_compressed_size(self) -> int: ...
    @property
    def statistics(self) -> ColumnChunkStatistics: ...

class RowGroup:
    def __init__(self): ...
    @property
    def columns(self) -> list[ColumnChunk]: ...
    @property
    def total_byte_size(self) -> int: ...
    @property
    def num_rows(self) -> int: ...
    @property
    def sorting_columns(self) -> list[SortingColumn] | None: ...
    @property
    def file_offset(self) -> int | None: ...
    @property
    def total_compressed_size(self) -> int | None: ...
    @property
    def ordinal(self) -> int | None: ...

class FileMetaData:
    def __init__(self): ...
    @property
    def version(self) -> int: ...
    @property
    def num_rows(self) -> int: ...
    @property
    def created_by(self) -> str: ...
    @property
    def row_groups(self) -> list[RowGroup]: ...
    @property
    def row_group_num_rows(self) -> list[int]: ...
    @property
    def columnchunk_metadata(self) -> dict[str, list[int]]: ...
    @classmethod
    def from_bytes(cls, footer_bytes: Buffer) -> FileMetaData: ...

def read_parquet_metadata(src_info: SourceInfo) -> ParquetMetadata: ...
def read_parquet_footers(src_info: SourceInfo) -> list[object]: ...
def read_parquet_column_chunk_bounds(file_metadatas: object, columns: object, stream: CudaStreamLike | None=None, mr: DeviceMemoryResource | None=None) -> Table: ...
